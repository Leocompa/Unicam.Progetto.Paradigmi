...
public class OrdineRepository : GenericRepository<Ordine>
{
        ...
        string email)
    {
        ...
        var risultatiPaginati = query.OrderBy(ordine => ordine.NumeroOrdine).Skip(from*num).Take(Math.Min(num,totalNum-from)).ToList();
    ...
    public List<Ordine> GetOrdine(int from, int num, string? filter, out int totalNum)
    {
        ...
        totalNum = query.Count();
        return query.OrderBy(ordine => ordine.NumeroOrdine).Skip(from*num).Take(Math.Min(num,totalNum-from)).ToList();
    }
}